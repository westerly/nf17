...
DROP VIEW public.alertevent;
DROP VIEW public.vventdepass;
DROP TABLE public.vents;
DROP VIEW public.alertetempressentie;
DROP VIEW public.vtempressentiedepass;
DROP VIEW public.alertetempreelle;
DROP VIEW public.vtempreelledepass;
DROP TABLE public.temperatures;
DROP VIEW public.alerteprecip;
DROP VIEW public.vprecdepass;
...
--
-- Name: vprecdepass; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW vprecdepass AS
    SELECT precipitations.precipitation_id, precipitations.force, precipitations.seuil, (precipitations.force - precipitations.seuil) AS depassement FROM precipitations;


ALTER TABLE public.vprecdepass OWNER TO postgres;

--
-- Name: alerteprecip; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW alerteprecip AS
    SELECT b.lieu_id, b.date, x.precipitation_id, x.force, x.seuil, x.depassement FROM (vprecdepass x JOIN bulletins b ON ((x.precipitation_id = b.precipitation_id))) WHERE (x.depassement > 0) ORDER BY x.depassement DESC;


ALTER TABLE public.alerteprecip OWNER TO postgres;
...
--
-- Name: vtempreelledepass; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW vtempreelledepass AS
    SELECT temperatures.temperature_id, temperatures.reelle, temperatures.seuil, (temperatures.reelle - temperatures.seuil) AS depassement FROM temperatures;


ALTER TABLE public.vtempreelledepass OWNER TO postgres;

--
-- Name: alertetempreelle; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW alertetempreelle AS
    SELECT b.lieu_id, b.date, x.temperature_id, x.reelle, x.seuil, x.depassement FROM (vtempreelledepass x JOIN bulletins b ON ((x.temperature_id = b.temperature_id))) WHERE (x.depassement > (0)::double precision) ORDER BY x.depassement DESC;


ALTER TABLE public.alertetempreelle OWNER TO postgres;

--
-- Name: vtempressentiedepass; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW vtempressentiedepass AS
    SELECT temperatures.temperature_id, temperatures.ressentie, temperatures.seuil, (temperatures.ressentie - temperatures.seuil) AS depassement FROM temperatures;


ALTER TABLE public.vtempressentiedepass OWNER TO postgres;

--
-- Name: alertetempressentie; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW alertetempressentie AS
    SELECT b.lieu_id, b.date, x.temperature_id, x.ressentie, x.seuil, x.depassement FROM (vtempressentiedepass x JOIN bulletins b ON ((x.temperature_id = b.temperature_id))) WHERE (x.depassement > (0)::double precision) ORDER BY x.depassement DESC;


ALTER TABLE public.alertetempressentie OWNER TO postgres;
...
--
-- Name: vventdepass; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW vventdepass AS
    SELECT vents.vent_id, vents.force, vents.seuil, (vents.force - vents.seuil) AS depassement FROM vents;


ALTER TABLE public.vventdepass OWNER TO postgres;

--
-- Name: alertevent; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW alertevent AS
    SELECT b.lieu_id, b.date, x.vent_id, x.force, x.seuil, x.depassement FROM (vventdepass x JOIN bulletins b ON ((x.vent_id = b.vent_id))) WHERE (x.depassement > (0)::double precision) ORDER BY x.depassement DESC;


ALTER TABLE public.alertevent OWNER TO postgres;
...
--
-- Name: vtempreelle; Type: VIEW; Schema: public; Owner: postgres
--

CREATE VIEW vtempreelle AS
    SELECT vtempreelledepass.temperature_id, vtempreelledepass.reelle, vtempreelledepass.seuil, vtempreelledepass.depassement FROM vtempreelledepass WHERE (vtempreelledepass.depassement > (0)::double precision) ORDER BY vtempreelledepass.depassement DESC;


ALTER TABLE public.vtempreelle OWNER TO postgres;
...
INSERT INTO capteurs VALUES (59, 'Bordeaux');
INSERT INTO capteurs VALUES (60, 'Toulouse');
INSERT INTO capteurs VALUES (61, 'Bordeaux');
...
SELECT pg_catalog.setval('capteurs_capteur_id_seq', 61, true);
...
INSERT INTO historiques VALUES (59, '2013-06-06 21:45:13.752', 'Bordeaux', NULL);
INSERT INTO historiques VALUES (60, '2013-06-06 21:45:36.261', 'Toulouse', NULL);
INSERT INTO historiques VALUES (61, '2013-06-06 21:47:59.424', 'Bordeaux', NULL);
...
INSERT INTO precipitations VALUES (58, 'Pluie', 10, 10, 'zeffijzefefhefhzefezifhef				
						 ...
						 ', 56);
INSERT INTO precipitations VALUES (56, 'Pluie', 9, 10, 'zeffijzefefhefhzefezifhef				
...
INSERT INTO temperatures VALUES (24, 12, 26, 25, 'frgrgeergregregregregregregrgrgerg				 ', 56);
INSERT INTO temperatures VALUES (22, 26, 27, 25, 'frgrgeergregregregregregregrgrgerg				 ', 58);
...
INSERT INTO vents VALUES (35, 10, 'Nord', 8, 'rezdfbzefgezufgzeufgezfuigefyugezfg				 ', 56);
